...
|`--style`|S3 寻址风格：`path` 或 `virtual-host`。默认根据 URL 格式自动推断。使用此参数可以覆盖自动推断结果。适用于 `s3://` 和 `minio://` 协议。|
...
|`--delete-dst, --deleteDst`|删除目标存储下的不相关对象。启用 `--db`/gate 时，源端被第一层 gate 跳过且目标端同 key 存在的对象不会被误判为 extra。|
...
|`--preserve-meta`|保留源对象的 Content-Type 和用户自定义 metadata，默认为 false。|
|`--db`|将同步/扫描结果记录到 MySQL，例如 `mysql://user:pass@host:3306/dbname`；启用两层 gate 时 DSN 需要带 database。|
|`--db-record-status`|指定哪些对象状态写入 `--db` 明细表，逗号分隔，默认为 `copied,failed`；普通同步支持 `copied/skipped/failed/deleted`，scan/scan-single 额外支持 `missing/differs/matches/extra`。|
|`--scan`|仅对比源和目标，不复制数据，结果写入 `--db` 或 `--output`。|
|`--scan-single`|仅扫描单个源桶，不访问目标，通过 ListObjects 记录 key/size/mtime/storage_class。|
|`--output`|将 scan/scan-single 结果输出到 CSV。|
|`--double-check`|正常同步结束后再执行一遍查漏。|
|`--fix-meta`|仅修复同 size 对象的 Content-Type/metadata，不复制数据；在正常同步流程前返回。|
|`--gate-table`|指定两层 gate 记录表；默认根据 src/dst 哈希生成 `sync_records_v2_<hash>`。|
