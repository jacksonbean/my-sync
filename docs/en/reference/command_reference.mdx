...
|`--style`|S3 addressing style: `path` or `virtual-host`. By default, the style is auto-detected from the URL format. Use this flag to override the auto-detection. Applies to `s3://` and `minio://` protocols.|
...
|`--delete-dst, --deleteDst`|Delete extraneous objects from destination. When `--db`/gate is enabled, objects skipped by the first gate with the same key on destination are not misclassified as extra.|
...
|`--preserve-meta`|Preserve source Content-Type and user-defined metadata, default to false.|
|`--db`|Record sync/scan results to MySQL, for example `mysql://user:pass@host:3306/dbname`; when the two-layer gate is enabled, the DSN must include a database name.|
|`--db-record-status`|Comma-separated object statuses to record in the `--db` detail table, default `copied,failed`; normal sync supports `copied/skipped/failed/deleted`, scan/scan-single additionally supports `missing/differs/matches/extra`.|
|`--scan`|Compare source and destination without copying; write results to `--db` or `--output`.|
|`--scan-single`|Scan a single source bucket only, without accessing destination, and record key/size/mtime/storage_class via ListObjects.|
|`--output`|Write scan/scan-single results to a CSV file.|
|`--double-check`|Run a second pass after sync to catch objects added during migration.|
|`--fix-meta`|Fix Content-Type/metadata for same-size objects without copying data; returns before the normal sync pipeline starts.|
|`--gate-table`|Custom table for two-layer gate records; defaults to `sync_records_v2_<hash>` generated from src/dst.|
