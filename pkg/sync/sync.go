...
// getSrcMeta retrieves metadata (content-type and user-defined headers) from source object.
func getSrcMeta(src object.ObjectStorage, key string) (object.ObjectMeta, bool) {
	srcObj, err := src.Head(ctx, key)
	if err != nil {
...
func copyData(src, dst object.ObjectStorage, key string, size int64, mtime time.Time, calChksum bool, uploads multipartUploads) (uint32, error) {
	...
	if size < maxBlock {
		...
		if upload == nil {
			// Try metadata-aware multipart upload (always for ContentType)
			if srcMeta, ok := getSrcMeta(src, key); ok {
				if !preserveMeta {
					srcMeta.Metadata = nil
				}
