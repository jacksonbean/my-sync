...
type Object interface {
	...
	StorageClass() string
	Status() string
	// ContentType returns the MIME type of the object (e.g. "application/json").
	// Returns empty string if not available.
	ContentType() string
	// Metadata returns the user-defined metadata of the object (e.g. x-amz-meta-* headers).
...
func (o *objWithMeta) ContentType() string         { return o.contentType }
func (o *objWithMeta) Metadata() map[string]string { return o.metadata }

// ObjectMeta carries metadata to be preserved during object operations.
type ObjectMeta struct {
	ContentType string
	Metadata    map[string]string
}

// MetadataPutter is an optional interface for ObjectStorage backends that
// support setting content-type and user-defined metadata when putting objects.
type MetadataPutter interface {
	PutWithMeta(ctx context.Context, key string, in io.Reader, meta ObjectMeta, getters ...AttrGetter) error
}

// MetadataMultipartCreator is an optional interface for ObjectStorage backends
// that support setting metadata when creating multipart uploads.
type MetadataMultipartCreator interface {
	CreateMultipartUploadWithMeta(ctx context.Context, key string, meta ObjectMeta) (*MultipartUpload, error)
}
