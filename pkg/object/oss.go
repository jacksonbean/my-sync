...
func (o *ossClient) Head(ctx context.Context, key string) (Object, error) {
	...
	// Extract user metadata (x-oss-meta-* headers)
	meta := make(map[string]string)
	for k, v := range info.Headers {
		if strings.HasPrefix(strings.ToLower(k), strings.ToLower(oss.HeaderOssMetaPrefix)) && len(v) > 0 {
			meta[k] = v[0]
		}
	}
	return &objWithMeta{
		obj: obj{
			...
			status: oss.ToString(info.Restore),
		},
...
func (o *ossClient) Put(ctx context.Context, key string, in io.Reader, getters ...AttrGetter) error {
	return o.put(ctx, key, in, ObjectMeta{}, getters...)
}

// PutWithMeta puts an object with the given metadata.
func (o *ossClient) PutWithMeta(ctx context.Context, key string, in io.Reader, meta ObjectMeta, getters ...AttrGetter) error {
	return o.put(ctx, key, in, meta, getters...)
}

func (o *ossClient) put(ctx context.Context, key string, in io.Reader, meta ObjectMeta, getters ...AttrGetter) error {
	...
	if meta.ContentType != "" {
		req.ContentType = oss.Ptr(meta.ContentType)
	}
...
func (o *ossClient) CreateMultipartUpload(ctx context.Context, key string) (*MultipartUpload, error) {
	return o.createMultipartUpload(ctx, key, ObjectMeta{})
}

// CreateMultipartUploadWithMeta creates a multipart upload with metadata.
func (o *ossClient) CreateMultipartUploadWithMeta(ctx context.Context, key string, meta ObjectMeta) (*MultipartUpload, error) {
	return o.createMultipartUpload(ctx, key, meta)
}

func (o *ossClient) createMultipartUpload(ctx context.Context, key string, meta ObjectMeta) (*MultipartUpload, error) {
	...
	result, err := o.client.InitiateMultipartUpload(ctx, req)
	if err != nil {
		return nil, err
	}
