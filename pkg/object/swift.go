...
func (s *swiftOSS) PutWithMeta(ctx context.Context, key string, in io.Reader, meta ObjectMeta, getters ...AttrGetter) error {
	return s.put(ctx, key, in, meta, getters...)
}

func (s *swiftOSS) put(ctx context.Context, key string, in io.Reader, meta ObjectMeta, getters ...AttrGetter) error {
	mimeType := meta.ContentType
	if mimeType == "" {
		mimeType = utils.GuessMimeType(key)
	}
	_, err := s.conn.ObjectPut(ctx, s.container, key, in, true, "", mimeType, meta.Metadata)
	return err
}
...
func (s *swiftOSS) Head(ctx context.Context, key string) (Object, error) {
	object, hdrs, err := s.conn.Object(ctx, s.container, key)
	if err != nil {
