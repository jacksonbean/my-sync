...
func (t *tosClient) Put(ctx context.Context, key string, in io.Reader, getters ...AttrGetter) error {
	return t.put(ctx, key, in, ObjectMeta{}, getters...)
}

// PutWithMeta puts an object with the given metadata.
func (t *tosClient) PutWithMeta(ctx context.Context, key string, in io.Reader, meta ObjectMeta, getters ...AttrGetter) error {
	return t.put(ctx, key, in, meta, getters...)
}

func (t *tosClient) put(ctx context.Context, key string, in io.Reader, meta ObjectMeta, getters ...AttrGetter) error {
	...
	resp, err := t.client.PutObjectV2(ctx, req)
	if resp != nil {
		attrs := ApplyGetters(getters...)
...
func (t *tosClient) CreateMultipartUpload(ctx context.Context, key string) (*MultipartUpload, error) {
	return t.createMultipartUpload(ctx, key, ObjectMeta{})
}

// CreateMultipartUploadWithMeta creates a multipart upload with metadata.
func (t *tosClient) CreateMultipartUploadWithMeta(ctx context.Context, key string, meta ObjectMeta) (*MultipartUpload, error) {
	return t.createMultipartUpload(ctx, key, meta)
}

func (t *tosClient) createMultipartUpload(ctx context.Context, key string, meta ObjectMeta) (*MultipartUpload, error) {
	req := &tos.CreateMultipartUploadV2Input{
		Bucket:       t.bucket,
		Key:          key,
	...
	resp, err := t.client.CreateMultipartUploadV2(ctx, req)
	if err != nil {
		return nil, err
	}
